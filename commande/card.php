...
/* --------------------------- NG bloquer reouverture si facture/commande fournisseur existe--------------------------------- */

	   $resql = $db->query("select fk_target from ".MAIN_DB_PREFIX."element_element where sourcetype = 'commande' and targettype = 'facture' and fk_source = '".$object->id."'");
	   $resql = mysqli_num_rows($resql);
	   if($resql>0){
			setEventMessages('Error : impossible de réouvrir cette commande', 'Supprimer la(es) facture(s) lié à cette commande', 'errors');
			$error++;
	   } else {

	   	$resql = $db->query("select dcf.rowid from ".MAIN_DB_PREFIX."commande_fournisseur_customfields as dcf where refcc = '".$object->ref."'");
		$resql = mysqli_num_rows($resql);
		if($resql>0){
			setEventMessages('Error : impossible de réouvrir cette commande', 'Supprimer la(es) commande(s) fournisseurs lié à cette commande', 'errors');
			$error++;
	   	} else {
...
/* ------------------------------------- NG bloquer suppression si facture/commande existe -------------------------------------*/
		if ($action == 'delete') {

		     	$resql = $db->query("select fk_target from ".MAIN_DB_PREFIX."element_element where sourcetype = 'commande' and targettype = 'facture' and fk_source = '".$object->id."'");
	   		$resql = mysqli_num_rows($resql);
	   		if($resql>0){
				setEventMessages('Error : impossible de supprimer cette commande', 'Supprimer la(es) facure(s) lié à cette proposition', 'errors');
				$error++;
	   		} else {
				$resql = $db->query("select dcf.rowid from ".MAIN_DB_PREFIX."commande_fournisseur_customfields as dcf where refcc = '".$object->ref."'");
				$resql = mysqli_num_rows($resql);
				if($resql>0){
					setEventMessages('Error : impossible de supprimer cette commande', 'Supprimer la(es) commmande(s) fournisseurs lié à cette proposition', 'errors');
					$error++;
	   			} else {
		...
		*/
		...
		if ($action == 'modif')
		{
			...
			$resql = $db->query("select fk_target from ".MAIN_DB_PREFIX."element_element where sourcetype = 'commande' and targettype = 'facture' and fk_source = '".$object->id."'");
	   		$resql = mysqli_num_rows($resql);
	   		if($resql>0){
				setEventMessages('Error : impossible de modifier cette commande', 'Supprimer la(es) facure(s) lié à cette proposition', 'errors');
				$error++;
	   		} else {
				$resql = $db->query("select dcf.rowid from ".MAIN_DB_PREFIX."commande_fournisseur_customfields as dcf where refcc = '".$object->ref."'");
				$resql = mysqli_num_rows($resql);
				if($resql>0){
					setEventMessages('Error : impossible de modifier cette commande', 'Supprimer la(es) commmande(s) fournisseurs lié à cette proposition', 'errors');
					$error++;
	   			} else {
