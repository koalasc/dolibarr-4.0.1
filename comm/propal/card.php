...
/* --------------------------- NG bloquer reouverture si commande existe--------------------------------- */

		$resql = $db->query("select fk_target from ".MAIN_DB_PREFIX."element_element where sourcetype = 'propal' and targettype = 'commande' and fk_source = '".$object->id."'");
		$resql = mysqli_num_rows($resql);
		if($resql>0){
			setEventMessages('Error : impossible de réouvrir cette proposition', 'Supprimer la(es) commmande(s) lié à cette proposition', 'errors');
			$error++;
		} else {
...
--------------------------------------------------------------------------------------------------------*/
				...
				// Delete 
				/*NG ( je sais plus pk ) if ($user->rights->propal->supprimer) { */
				if ($user->rights->propal->supprimer && $object->statut == Propal::STATUS_DRAFT) {
...
/* ------------------------------------- NG include pour simplifié library ------------------------------------- */
