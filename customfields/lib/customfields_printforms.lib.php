 ...
 *      @param      idvar                       the name of the POST or GET\A0variable containing the id of the object
    ...
    if ($customfields->probeTable()) { // ... and if the table for this module exists, we show the custom fields
        ...
        if (isset($fields)) { // only if at least one customfield exists (there's a special case where a record may exist because there existed customfields, but all customfields were deleted, and thus the records still exist with a rowid and fk_module columns set, but with nothing else. In this case, we skip.)
            ...
            foreach ($fields as $field) { // for each customfields, we will print/save the edits
                ...
                // print the edit button only if authorized //NG bloquer modif si le status n'est pas brouillon
                if (! empty($object->brouillon) &&!($action == 'editcustomfields' && strtolower(GETPOST('field')) == $name) && !(isset($objet->brouillon) and $object->brouillon == false) && $rightok && !$field->extra['noteditable'] && !$hidecondition) $out .= '<span align="right"><a style="float:right;margin-right:-1px">&nbsp;</a><a href="'.$_SERVER["PHP_SELF"].'?'.$idvar.'='.$objid.'&amp;action=editcustomfields&amp;field='.$field->column_name.'">'.img_edit("default",1).'</a></td>';
                $out .= '</td>';
